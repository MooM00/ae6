	...
	document.querySelectorAll("script[type='text/perl']")
		.forEach(function (el) {
			...
			for (i=0; i<el.attributes.length; i++) {
			    ...
			    if (attr.name == 'path') { path=attr.value.replace(/\\/g,'/'); }
			    if (attr.name == 'filename') { name=attr.value.replace(/\\/g,'/'); }
			    if (num=attr.name.match(/^argv(\d+)$/)) { largv[num[1]]=attr.value; }
			    if (num=attr.name.match(/^cli_argv(\d+)$/)) { pargv[num[1]]=attr.value; }
			}
			Perl.runParameters.push.apply(Perl.runParameters,largv);
			Perl.cliParameters.push.apply(Perl.cliParameters,pargv);

			if (el.src) { 
			    if ( name === 'inline.pl' ) {
				name=el.src.replace(/\\/g,'/').substr(el.src.lastIndexOf('/')+1); 
			    }
			    ajax_counter+=1;
			    xhr[ajax_counter]= new XMLHttpRequest();
			    xhr[ajax_counter].addEventListener("load",function () {
				console.debug ("Perl: received file [",this.saveto,"]");
				scripts[this.saveto] = this.responseText;
				ajax_counter-=1;
				if (ajax_counter==0) {
				    console.debug("Perl: All AJAX done. autorunning '", Perl.fileToRun, "'...");
				    Perl._saveAndRun(scripts);
				}
			    });
			    xhr[ajax_counter].open("GET",el.src);
			    xhr[ajax_counter].saveto=path+'/'+name;
			} else {
			    if (scripts[path+'/'+name] === undefined) { scripts[path+'/'+name]=''; }
			    scripts[path+'/'+name]+=el.innerHTML;
			}

			if ( 
			    (el.attributes.runme && el.attributes.runme.value) || 
			    (Perl.fileToRun === undefined && name.slice(-3) == '.pl')
			) { Perl.fileToRun=path+'/'+name }
		});
	if (ajax_counter) {
	    console.debug("Perl: Found ", ajax_counter, " src= scripts. Loading...");
	    Object.keys(xhr).map(function(key,index) { xhr[key].send(); });
	} else {
	    if (Perl.fileToRun !== undefined) {
		console.debug("Perl: Found only inline scripts. autorunning '", Perl.fileToRun, "'...");
		Perl._saveAndRun(scripts);
	    } else console.debug("Perl: No embedded scripts");
	}
});

Perl._ensureWebPerl = function (code) { // maybe rather use code.split('\n') sothat we can ignore comments and blank lines?
    console.debug("Perl: ensuring webperl.pm....");
    var codelines=code.split("\n");
    var first=0;
    const foundit = /^\s*use\s+WebPerl(\s|;)/m;
    const findfirst = /^\s*(use|my|our|local|package)/m;
    const findskip = /^\s*(#|$)/m;
    var i;
    for (i=0; i<codelines.length; i++) {
	if (foundit.exec(codelines[i])) { return code; }
	if (! first && findfirst.exec(codelines[i])) { //add 'use' above here if it isn't found
	    first=i;
	}
	if (! findskip.exec(codelines[i])) { // we don't want to add below here, so either here or where we found earlier 
	    if (first==0) {first=i;}
	    console.debug("Perl: Autoloading WebPerl");
	    codelines.splice(first,0,"use WebPerl 'js';"); return codelines.join("\n"); 
	}
    };
    return code;
}

Perl._saveAndRun = function (scripts) {
	Perl.init(function () {
	        scripts[Perl.fileToRun]=Perl._ensureWebPerl(scripts[Perl.fileToRun]);
		FS.mkdir('/webperl');
		FS.chdir('/webperl');
		Object.keys(scripts).map(function(key,index) {
		    try {
			console.debug("Perl: saving file [",key,"] for ",scripts[key].length , " bytes...");
			// looks like neither mkdir nor mkdirTree do './thing' like a shell... or like writeFile,etc.
			// so we need to sort that all out first.
			var mkpath=key; 
			if (key.slice(0,2)=='./') {
			    mkpath=FS.cwd()+key.slice(1);
			} else if (key.slice(0,1) != '/') {
			    mkpath=FS.cwd()+'/'+key;
			}
			mkpath=mkpath.slice(0,mkpath.lastIndexOf('/'));
			if (mkpath !=='') {
			    FS.mkdirTree(mkpath); //Undocumented, but in the code. 
			}
			FS.writeFile(key,scripts[key]);
		    }
		    catch (err) { console.error("Perl:",err); alert("Save to "+key+" failed: "+err); }
		});

		try {
			console.debug("Perl: Saved script(s). Now running ...")
			...
			Perl.runParameters.unshift(Perl.fileToRun);
			Perl.runParameters.unshift('.');
			Perl.runParameters.unshift('-I');
			Perl.cliParameters.concat(Perl.runParameters);
			console.debug("Perl:                              ...",Perl.cliParameters.concat(Perl.runParameters))
			window.setTimeout(function () { Perl.start( Perl.cliParameters.concat(Perl.runParameters) ); }, 1);
		}
		catch (err) { console.error("Perl:",err); }
	});
};
